...
public static class ScenesMenuBuild
{
    ...
    static readonly string ScenesMenuPath = "Script/Editor/ScenesMenu.cs";

    [MenuItem("Scenes/Update all scenes list for hotkey Switch")]
    public static void UpdateList()
    {
        ...
        foreach (string sceneGuid in AssetDatabase.FindAssets("t:Scene", new string[] { "Assets" }))
        {
            ...
            string methodName = sceneFilename.Replace('/', '_').Replace('\\', '_').Replace('.', '_').Replace('-','_').Replace(" ","");
        ...
        stringBuilder.AppendLine("}");
        Debug.Log(Path.GetDirectoryName(scenesMenuPath)+">>>>>>>****");
